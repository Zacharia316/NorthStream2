...
// Embedded directly (not parsed from any M3U) — always merged into the channel list.
const CUSTOM_CHANNELS = [
  {
    id: "yt-supersport",
    name: "SuperSport",
    url: "https://www.youtube.com/embed/DYfDupEsjZE?autoplay=1&mute=0",
    logo: "",
    country: "",
    category: "Football",
    quality: "HD",
  ...
  },
  {
    id: "yt-sportytv",
    name: "Sporty TV",
    url: "https://www.youtube.com/embed/T64JTcJZG5o?autoplay=1&mute=0",
    logo: "",
    country: "",
    category: "Football",
    quality: "HD",
  ...
  },
  {
    id: "yt-cnbc",
    name: "CNBC",
    url: "https://www.youtube.com/embed/FVO4fLyMiXk?autoplay=1&mute=0",
    logo: "",
    country: "",
    category: "Football",
    quality: "HD",
...
function parseM3U(text) {
  // Trim + drop blank lines first — prevents garbage entries getting treated as stream URLs
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("#EXTINF")) {
      ...
      // Only accept real http(s) URLs — rejects blank/comment lines that sneak through
      current.url = line;
...
async function getHls() {
  ...
  const mod = await import("https://cdn.jsdelivr.net/npm/hls.js@1.5.7/dist/hls.min.js");
  _hlsModule = mod.default || window.Hls;
  return _hlsModule;
}
// Kick off the load immediately so it's ready by the time user taps a channel
getHls().catch(() => {});
...
export default function NorthStreamHome() {
  ...
  const fetchSource = useCallback(async (url) => {
    ...
    try {
      ...
      setChannels([...parsed, ...CUSTOM_CHANNELS]);
      setLoadState("ok");
    } catch (e) {
  ...
  /* ---- HLS.js player ---- */
  const HLS_CFG = {
    // Buffer: keep short so first segment plays immediately, grow later
    maxBufferLength: 10, maxMaxBufferLength: 60,
    // Start at lowest quality for fast initial load, let ABR ramp up
    startLevel: -1, // -1 = auto (picks based on bandwidth estimate)
    abrEwmaDefaultEstimate: 1500000, // generous starting estimate → faster ramp
    testBandwidth: true,
    progressive: true,
    lowLatencyMode: false,
    // Tighter timeouts — fail fast, don't stall 8s on dead segments
    fragLoadingTimeOut: 4000, manifestLoadingTimeOut: 6000,
    fragLoadingMaxRetry: 2, manifestLoadingMaxRetry: 2,
    // Use web worker for segment parsing — offloads main thread
    enableWorker: true,
    // Start fetching next fragment sooner
  ...
  const openChannel = (ch) => {
    ...
    setHlsError(false);
    setIsRetrying(false);
    if (!ch.isYouTube) setTimeout(() => loadStream(ch.url), 80);
  };
      ...
      {playerState === "full" && activeChannel && (
        <div className="ns-fade" style={{
          ...
          >
            ...
            <video
              ref={videoRef}
              style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "contain", display: activeChannel?.isYouTube ? "none" : "block" }}
              playsInline
              onPlay={() => setIsPlaying(true)}
              onPause={() => setIsPlaying(false)}
            />

            {/* YouTube embed — used for channels sourced from a YouTube link instead of an HLS stream */}
            {activeChannel?.isYouTube && (
              <iframe
